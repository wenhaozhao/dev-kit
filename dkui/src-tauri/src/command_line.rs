...
lazy_static! {
    static ref DEVKIT_PATH: AtomicPtr<PathBuf> = {
        match which::which("devkit") {
            ...
            Err(_) => AtomicPtr::new(ptr::null_mut()),
        }
    };
}
...
pub fn add_to_path() -> Result<String, String> {
    ...
    #[cfg(any(target_os = "macos", target_os = "linux"))]
    {
        ...
        let shell = get_current_shell()
            .ok_or("Failed to get current shell")?
            .to_lowercase();
        match shell.as_str() {
            "fish" => {
...
alias dk="devkit"
        ...
        DEVKIT_PATH.store(
            Box::into_raw(Box::new(bin_dir.join("devkit"))),
            Ordering::Relaxed,
        );
        Ok("add devkit to PATH ok, you can use devkit or dk in command-line now!!!".to_string())
    }
    #[cfg(target_os = "windows")]
    {
        return Err("Unsupported operating system".to_string());
